...
const SelectColor:React.FC<SelectColorProps> = ({item, addImageToState, removeImageFromState, isProductCreated}) => {
    ...
    const handleFileChange = useCallback((value: File) =>{          //handling file change
        setFile(value);
    ...
    return (  
        <div className="grid grid-cols-1 md:grid-cols-2 overflow-y-auto border-[1.2px] border-slate-200 items-center p-2">
            <div className="flex flex-row gap-2 items-center h-[60px]">
                ...
                <label htmlFor={item.color} className="cursor-pointer font-medium">{item.color}</label>
            </div>
            <>
            {isSelected && !file && (
                <div className="col-span-2 text-center">    {/*image selection */}
