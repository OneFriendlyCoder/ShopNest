...
export async function POST(request: Request){
    ...
    const orderData = {                                     // this data would be stored in the DB for order
    ...
    if(payment_intent_id){          //payment_intent_id we get from the request body
        ...
        // create the payment intent also create the order
        const paymentIntent = await stripe.paymentIntents.create({
            amount: total,
            currency: 'usd',
            automatic_payment_methods: {enabled: true},
            description: "Software development services"                    //As per Indian standards it is required to add description as well
        });
        // create the order
